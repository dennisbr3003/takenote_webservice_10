...
public class H2DatabaseBusinessObject implements 
...
{ 
	...
	private String getTableDefinition(String tableName) {
		...
		case TABLE_NTS:
			return String.format("CREATE TABLE IF NOT EXISTS %s (%s VARCHAR(100) NOT NULL, %s VARCHAR(50) NOT NULL, " + 
					"%s VARCHAR(40) NOT NULL, %s BLOB, %s VARCHAR(50), %s VARCHAR(30), %s VARCHAR(30), %s DATETIME NOT " + 
					"NULL DEFAULT CURRENT_TIMESTAMP, %s DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, " + 
					"CONSTRAINT %s PRIMARY KEY (%s,%s));", 
		...
		case TABLE_USR:
			return String.format("CREATE TABLE IF NOT EXISTS %s (%s VARCHAR(100) NOT NULL, %s VARCHAR(255) NOT NULL, %s VARCHAR(255) NOT NULL, " + 
					"%s VARCHAR(255), CONSTRAINT %s PRIMARY KEY (%s, %s));", 
					TABLE_USR, USR_NAME, USR_PASSWRD, USR_DID, USR_REMARK, P_KEY_USR, USR_NAME, USR_DID);				
		default:
			return "";
		}
	}
	...
	private boolean createTable(String tableName) {
		...
		try {
			Statement statement = H2Database.getInstance().getConnection().createStatement();
